...
const menuItems: { text: string; asDialogClose?: boolean }[] = [
  { text: "Resume", asDialogClose: true },
  { text: "Options" },
  { text: "Extras" },
  { text: "Quit" },
];
      ...
      <Dialog.Portal>
        <Dialog.Popup className="fixed inset-0 flex items-center justify-center">
          ...
          <ul className="font-menu flex flex-col gap-y-2 text-center text-4xl font-bold tracking-wide text-[#1a72c8] [-webkit-text-stroke:5px_rgba(0,0,0,0.75)] [paint-order:stroke_fill]">
            {menuItems.map(({ text, asDialogClose }, index) => (
              <MenuItem
                ref={(item) => {
                  menuItemRefs.current[index] = item;
                }}
                key={index}
                asDialogClose={asDialogClose}
...
function MenuItem({ asDialogClose, children, ...props }: MenuItemProps) {
  ...
  const Comp = asDialogClose ? Dialog.Close : "button";
